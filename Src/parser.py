...
class Parser(object):
    def __init__(self):
        ...
        parser.add_argument(
            "--algo_name",
            default='DSPO_clip',
            help="Policy/algorithm used, capital sensitive",
            choices=[
                # 2x2 DSPO diagnostic experiment
                'DSPO_clip',
                'DSPO_wide',
                'DSPO_PLUS_clip',
                'DSPO_PLUS_wide',
                # Backward-compatible / other algorithms
                'DSPO',
                'DSPO_PLUS',
                'Heuristic',
                'Baseline',
    ...
    def environment_parameters(self, parser):
        ...
        parser.add_argument(
            "--wide_price_bound",
            default=1000.0,
            help="Large finite bound M for DSPO_wide and DSPO_PLUS_wide, giving price range [-M, M].",
            type=float,
        )
    ...
    def DSPO_parameters(self, parser):
        ...
        # DSPO_PLUS / SPO+ parameters.  Ignored by pure DSPO variants.
        parser.add_argument("--spo_warmup_episodes", default=5, help="Warmup episodes before enabling SPO+ loss", type=int)
        parser.add_argument("--spo_rampup_episodes", default=10, help="Ramp-up episodes for SPO+ loss weight", type=int)
        parser.add_argument("--spo_loss_weight", default=0.3, help="Maximum SPO+ loss weight in mixed objective", type=float)
        parser.add_argument("--spo_loss_scale", default=1.0, help="Optional scale multiplier for SPO+ loss", type=float)
        parser.add_argument("--spo_batch_size", default=64, help="Replay mini-batch size for SPO+ loss", type=int)
        parser.add_argument("--spo_replay_size", default=500, help="Replay buffer size for SPO+ decision records", type=int)
        parser.add_argument("--spo_grad_clip_norm", default=5.0, help="Gradient clipping norm for DSPO_PLUS", type=float)
