...
class Heuristic(Agent):
    ...
    def get_action_offer(self,state,training):
        ...
        for pp in range(len(pps)):
            if state[2]["parcelpoints"][pp].remainingCapacity > 0:#check if parcelpont has remaining capacity
                ...
                pp_costs[pp] = self.l_mp*mltplr + mltplr*((1-theta)*self.cheapestInsertionCosts(state[2]["parcelpoints"][pp].location, state[1]) + theta*self.historicCosts(state[2]["parcelpoints"][pp].location,self.historicRoutes))
    ...
    def get_action_pricing(self,state,training):
        ...
        # state[0].service_time stores l_addr(i) in seconds, need to add l0_home
        # Convert to minutes for wage calculation: (l0_home + l_addr(i)) / 60
        home_service_time_minutes = (self.l0_home + state[0].service_time) / 60
        homeCosts = home_service_time_minutes*self.wage+mltplr*((1-theta)*(self.cheapestInsertionCosts(state[0].home, state[1]) ) + theta*(self.historicCosts(state[0].home,self.historicRoutes) ))
        
        # 使用customerchoice模型的MNL计算HOME delivery效用（如果可用）
        # 注意：Heuristic算法不支持在途时间预测，所以travel_time为None
        ...
        for idx,pp in enumerate(pps):
            if pp.remainingCapacity > 0:
                ...
                if customerchoice_model is not None:
                    util = customerchoice_model.mnl(state[0], pp, travel_time=None)
                else:
                    util = self.mnl(state[0],pp)
                # OOH point cost: includes service time (l_mp) and routing cost
                pp_costs[idx] = self.l_mp*mltplr + mltplr * ((1-theta)* ( self.cheapestInsertionCosts(pp.location, state[1]) )+ theta* ( self.historicCosts(pp.location,self.historicRoutes) ))
                sum_mnl += exp(util+(state[0].incentiveSensitivity*(pp_costs[idx]-self.revenue)))
       
        # 添加外部选项到MNL分母（如果存在）- 确保与环境的MNL模型一致
    ...
    def mnl_euclid(self,customer,parcelpoint):
        ...
        beta_p = -distance / max(self.dist_scaler, 1e-8)
