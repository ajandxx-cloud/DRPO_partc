...
## Status for the DRT manuscript

This workspace is currently a local experimental codebase derived from the original out-of-home delivery repository above. It has been reused to prototype the DRT manuscript's `DSPO` and `DRPO` experiments, but it is not yet the cleaned public reproducibility package referenced by the revised manuscript. For that reason, the manuscript now relies on explicit parameter tables, pseudocode, and archived result summaries rather than citing this repository as a finalized public artifact.
...
* Baseline.py: Contains the tuned static-pricing baseline used as `Static-pricing` in the revised manuscript.
* DSPO.py: Contains the original Dynamic Selection and Pricing of OOH method (DSPO).
* DRPO.py: Public entry point for the DSPO + SPO variant used as `DRPO`.
* DSPO_plus_SPO.py: Legacy implementation class kept for backward compatibility with old logs and scripts.
...
* Predictors.py: Contains the prediction models used for DSPO and DRPO.
* Utils.py: Contains several helper functions such as plotting.

Current experiment naming used in this repo and the revised manuscript:
- `Static-pricing` = `Baseline.py`
- `DSPO` = `DSPO.py`
- `DRPO` = `DRPO.py` (thin wrapper around the legacy `DSPO_plus_SPO` implementation)
...
## GPU Execution (Windows, strict mode)

This repository currently has mixed Python runtimes on some machines (CPU-only default Python and CUDA-enabled Python38).  
To enforce strict GPU usage:

1. Check GPU runtime:
   - `powershell -ExecutionPolicy Bypass -File scripts/gpu_env_check.ps1`
2. Run RC full12 OAT with fixed GPU interpreter:
   - `powershell -ExecutionPolicy Bypass -File scripts/run_rc_full12_gpu.ps1`
3. Run only smoke validation:
   - `powershell -ExecutionPolicy Bypass -File scripts/run_rc_full12_gpu.ps1 -OnlySmoke`
4. Run a single `DRPO` training command with GPU:
   - `powershell -ExecutionPolicy Bypass -File scripts/run_single_train_gpu.ps1 -- --max_episodes 80 --seed 0 --data_seed 0 --data_seed_test 1`

Notes:
- The scripts pin Python to `C:/Users/39583/AppData/Local/Programs/Python/Python38/python.exe`.
- Strict GPU mode is enabled by default (no CPU fallback).
