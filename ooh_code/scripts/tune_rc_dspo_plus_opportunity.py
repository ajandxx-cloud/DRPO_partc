#!/usr/bin/env python3
"""Small-sample RC tuning for opportunity-cost lifted DRPO.

Runs a DSPO baseline and a small-weight SPO fine-tuning grid for
DRPO on three RC seeds. The SPO+ term is interpreted as regret for
an opportunity-cost-based local pricing oracle, not as a direct episode-level
black-box optimizer. Outputs candidate_raw.csv, candidate_summary.csv,
...
SPO_CANDIDATES: Dict[str, Dict[str, Any]] = {
    "alpha_002": {"spo_loss_weight": 0.02, "spo_warmup_episodes": 80, "spo_rampup_episodes": 30},
    "alpha_005": {"spo_loss_weight": 0.05, "spo_warmup_episodes": 80, "spo_rampup_episodes": 30},
    "alpha_010": {"spo_loss_weight": 0.10, "spo_warmup_episodes": 80, "spo_rampup_episodes": 30},
    "alpha_015": {"spo_loss_weight": 0.15, "spo_warmup_episodes": 80, "spo_rampup_episodes": 30},
    "alpha_020": {"spo_loss_weight": 0.20, "spo_warmup_episodes": 100, "spo_rampup_episodes": 40},
}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tune opportunity-cost lifted DRPO on a small RC sample.")
    ...
    p.add_argument("--run_prefix", default="RC_DRPO_OPPORTUNITY")
    p.add_argument("--folder_suffix", default="_drpo_opportunity_smoke")
...
def main() -> None:
    ...
    if not args.output_dir:
        args.output_dir = f"Experiments/analysis/rc_drpo_opportunity_smoke_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    meta = {
        ...
        "interpretation": (
            "DRPO uses lifted SPO+ for the local pricing oracle. "
            "The cost label is treated as an opportunity-cost proxy for the "
            "Bellman continuation-value loss, and SPO+ is used only as "
            "small-weight fine-tuning after Huber pretraining."
        ),
    }
