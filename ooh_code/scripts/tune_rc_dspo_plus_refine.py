#!/usr/bin/env python3
"""Refined RC tuning for opportunity-cost lifted DRPO.

Runs DSPO and refined low-impact DRPO candidates. This search is
designed after the first opportunity-cost sweep showed that SPO improves some
seeds but hurts seed 97; candidates therefore use lighter SPO weights, later
warmup, and optionally lower learning rates to preserve the Huber baseline.
"""
import argparse
import csv
import json
...
SPO_CANDIDATES: Dict[str, Dict[str, Any]] = {
    "a003_w120_r40": {
        "spo_loss_weight": 0.003,
        "spo_warmup_episodes": 120,
        "spo_rampup_episodes": 40,
    },
    "a005_w120_r40": {
        "spo_loss_weight": 0.005,
        "spo_warmup_episodes": 120,
        "spo_rampup_episodes": 40,
    },
    "a008_w120_r40": {
        "spo_loss_weight": 0.008,
        "spo_warmup_episodes": 120,
        "spo_rampup_episodes": 40,
    },
    "a010_w120_r40": {
        "spo_loss_weight": 0.010,
        "spo_warmup_episodes": 120,
        "spo_rampup_episodes": 40,
    },
    "a005_w150_r30": {
        "spo_loss_weight": 0.005,
        "spo_warmup_episodes": 150,
        "spo_rampup_episodes": 30,
    },
    "a010_w150_r30": {
        "spo_loss_weight": 0.010,
        "spo_warmup_episodes": 150,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Refine opportunity-cost lifted DRPO on RC.")
    ...
    p.add_argument("--run_prefix", default="RC_DRPO_REFINE")
    p.add_argument("--folder_suffix", default="_drpo_refine_smoke")
...
def main() -> None:
    ...
    if not args.output_dir:
        args.output_dir = f"Experiments/analysis/rc_drpo_refine_smoke_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    meta = {
        ...
        "interpretation": (
            "Refined search after the first opportunity-cost sweep. "
            "Candidates use lower SPO weights and later warmup so SPO+ acts "
            "as a small decision-focused correction to the Huber-trained CNN."
        ),
    }
